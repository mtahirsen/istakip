...
(function($){
	$.fn.extend({
		popConfirm: function(options) {
			var defaults = {
				title: 'Onay',
				content: 'Bu İşlemi Yapmak İstediğinize Emin Misiniz ?',
				placement: 'top'
			};
				...
				self.popover({
					...
					content: options.content + '\
						<p class="button-group" style="margin-top: 10px; text-align: center;">\
							<button type="button" class="btn btn-small btn-danger confirm-dialog-btn-confirm">Evet</button>\
							<button type="button" class="btn btn-small confirm-dialog-btn-abord">Hayır</button>\
						</p>\
					'
				}).click(function(e) {
...
function seo_write(){
	$.ajax({
		...
		type: 'POST',
		url: 'process.php?dispatch=general.seo&no_settings=1',
...
function seo_in_write(){
	$.ajax({
		data: {title: $('[name=seo]').val()},
		type: 'POST',
		url: 'process.php?dispatch=general.seo&no_settings=1',
		success: function(res){
			$('[name=seo]').val(res);
...
if($('[name=title]').length>0 && $('[name=seo]').length>0){
