...
if(!$site->musteri_online()) $core->location("client_login.php");
$online_admin = $sql->get_row("musteriler","*","WHERE id='".$_SESSION["client_id"]."'");
if($_GET["manage"]==""){
	$dispatch = "musteriler.home";
	$_GET["s"] = "display";
	$_GET["manage"] = "musteriler.home";
}else{
	$dispatch = $_GET["manage"];
}
...
<head>
	...
	<!--[if !IE]> -->
		<script type="text/javascript"> window.jQuery || document.write('<script src="libs/jquery.js">'+"<"+"/script>"); </script>
	<!-- <![endif]-->
	<!--[if lte IE 9]>
		<script type="text/javascript"> window.jQuery || document.write('<script src="libs/jquery.js">'+"<"+"/script>"); </script>
	<![endif]-->
...
<body class="theme-default main-menu-animated <? if($online_admin["fixed_top_menu"]==1) echo ' main-navbar-fixed'; if($online_admin["fixed_left_menu"]==1) echo ' main-menu-fixed'; ?>">
				...
				<div id="main-navbar-collapse" class="collapse navbar-collapse main-navbar-collapse">
					<div>
						<ul class="nav navbar-nav">
							<?php 
							foreach($client_top_menu_links as $module=>$infos){ 
							if($admin->permission($infos["link"],$online_admin["group_id"])==1){
							?>
							<li>
								<a href="?s=display&manage=<?=$infos["link"]?>"><i class="fa <?=$infos["icon"]?>"></i> &nbsp; <?=$infos["title"]?></a>
							</li>
							<?php } } ?>
						</ul>
						<div class="right clearfix">
							<ul class="nav navbar-nav pull-right right-navbar-nav">
								...
								<?php } ?>
								<li class="">
									<a href="?s=process&manage=musteriler.logout">
										Çıkış Yap
									</a>
								</li>
							</ul>
